...
<c:forEach items="${dicmeta}" var="entry">
	<c:choose>
		<c:when test="${value eq entry.code}">
		    <label class="radio ${inline}">
		      <input type="radio" name="${name}" value="${entry.code}" checked="checked" > ${entry.name}
		    </label>
		</c:when>
		<c:otherwise>
			<label class="radio ${inline}">
		      <input type="radio" name="${name}" value="${entry.code}" /> ${entry.name}
		    </label>
		</c:otherwise>
	</c:choose>
</c:forEach>
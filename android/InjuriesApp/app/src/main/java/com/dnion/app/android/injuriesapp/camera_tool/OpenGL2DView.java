...
// import com.orbbec.NativeNI.AbNativeUtils;
...
public class OpenGL2DView extends GLSurfaceView {
...
//    protected int mTextureHeight = 0;
    ...
    public OpenGL2DView(Context context) {
        super(context);
        init();
    }

    public OpenGL2DView(Context context, AttributeSet attrs){
        super( context,  attrs);
        init();
    }
    ...
    private void init() {
        ...
        setRenderer(new Renderer() {
            ...
            public void onSurfaceChanged(GL10 gl, int w, int h) {
                synchronized (OpenGL2DView.this) {
                    mSurfaceWidth = w;
                    mSurfaceHeight = h;
                }
            }
     ...
     * @param stream The frame to be drawn
     */
    synchronized public void update(VideoStream rgbStream,VideoStream depthstream) {
        mCurrFrameWidth = rgbStream.getVideoMode().getResolutionX();
        mCurrFrameHeight = rgbStream.getVideoMode().getResolutionY();
        ...
        VideoFrameRef frame  = rgbStream.readFrame();


        ByteBuffer byteBuf = frame.getData();

        //AbNativeUtils.ConvertTORGBA(byteBuf, mTexture, mCurrFrameWidth, mCurrFrameHeight,  frame.getStrideInBytes());
        AbNativeUtils.RGB888TORGBA(byteBuf, mTexture, mCurrFrameWidth, mCurrFrameHeight,  frame.getStrideInBytes());
